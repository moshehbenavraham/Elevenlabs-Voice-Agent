...
test('one close operation wins across participant departure, error and timeout', async () => {
  const reasons = [];
  const lifetime = createSessionLifetime(60, async (reason) => { reasons.push(reason); });
...
test('duration closes independently of a browser request', async (t) => {
  ...
  const reasons = [];
  const lifetime = createSessionLifetime(30, async (reason) => { reasons.push(reason); });
...
test('shutdown disposes the duration timer', async (t) => {
  t.mock.timers.enable({ apis: ['setTimeout'] });
  let closes = 0;
  const lifetime = createSessionLifetime(30, async () => { closes++; });
  lifetime.dispose(); t.mock.timers.tick(60000); await Promise.resolve();
  assert.equal(closes, 0);
});

test('worker uses the same bounded session configuration as the server', () => {
  const env = { LIVEKIT_URL: 'wss://demo.livekit.cloud', LIVEKIT_API_KEY: 'key', LIVEKIT_API_SECRET: 'secret' };
  assert.equal(getLiveKitConfig(env).maxSessionSeconds, 600);
  assert.equal(getLiveKitConfig({ ...env, LIVEKIT_SESSION_MAX_SECONDS: '99999' }).configured, false);
});
