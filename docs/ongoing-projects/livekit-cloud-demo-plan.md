...
**Implementation work, not missing user access:** build the local agent, app integration, and demo process orchestration described above. The CLI and working room credentials provide a usable starting point; the user does not need to supply a prebuilt agent.

**External readiness not yet proven:** project Inference access and available credits/billing. Check them through the initial local agent run; involve the user only if an actual billing/account restriction requires their action. Do not claim another provider API key is missing unless the selected pipeline actually requires one.

**Tunnel readiness to verify during implementation:** current ngrok login, tunnel availability, and external browser access. The existing scripts and user's prior demo workflow establish the intended setup, but this planning pass has not opened or tested a tunnel. No hosting target is missing for this scope.
